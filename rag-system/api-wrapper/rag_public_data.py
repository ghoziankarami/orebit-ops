...
Data sources:
- /workspace/rag-system/.vector_db/papers
  - collection: papers
...
class SafePaperRagStore:
    ...
    def browse(self, page: int = 1, limit: int = 24) -> dict[str, Any]:
        ...
        limit = max(1, min(500, int(limit or 24)))
        papers = self.papers()
        total = len(papers)
        start = (page - 1) * limit
        end = start + limit
...
class PaperRagStore:
    ...
    def answer(self, query: str, top_k: int = 5, history: list | None = None) -> dict[str, Any]:
        ...
        if not api_key:
            return {
                'query': query,
                'answer': 'LLM is not active in this environment. Context retrieval works, but the OpenRouter API key is not available.',
                'sources': results[:top_k],
                'llm_used': False,
            }
